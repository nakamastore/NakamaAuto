...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args.length < 2) {
      return message.reply(`${megaphone} Please provide the current product code and the new code.`);
    }
    ...
    try {
      ...
      if (existingProduct) {
        return message.reply(`${no} A product with the new code already exists.`);
      }
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#0099ff')
            .setDescription(`${verif} The code of product **${currentCode}** has been changed to **${newCode}**.`)
        ]
      });
    } catch (error) {
