...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args.length < 2) {
      return message.reply(`${megaphone} Please provide a product code and the new name.`);
    }
    ...
    if (!newName) {
      return message.reply(`${megaphone} Please provide a new name for the product.`);
    }
    ...
    try {
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#0099ff')
            .setDescription(`${verif} The name of product **${code}** has been changed to **${newName}**.`)
        ]
      });
    } catch (error) {
