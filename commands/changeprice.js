...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args.length < 2) {
      return message.reply(`${megaphone} Please provide a product code and the new price.`);
    }
    ...
    if (isNaN(newPrice) || newPrice <= 0) {
      return message.reply(`${warning} Please provide a valid price greater than 0.`);
    }
    ...
    try {
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#0099ff')
            .setDescription(`${verif} The price of product **${code}** has been changed to **${newPrice}**.`)
        ]
      });
    } catch (error) {
